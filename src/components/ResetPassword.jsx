...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
   ...
   try {
  const res = await axiosInstance.post("/account/reset-password", {
    token,
  ...
  const loginType = res.data?.data?.loginType;

  setMessage("Password reset successful. Redirecting to login...");

  setTimeout(() => {
    navigateToLogin(loginType);
  }, 2000);
  ...
  const navigateToLogin=(loginType)=>
  {
    switch (loginType) {
        case "USER":
            navigate("/login");
